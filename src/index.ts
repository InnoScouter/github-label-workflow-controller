...
async function executeWorkflowsWhenAddLabel({
  ...
  const labeledPRs = response.data.filter(
    (pr) =>
...
const handlePullRequestLabeled = async (
  ...
  context.log.info(
    `Label ${label} was added to pull request #${pullRequestNumber}`,
  );
...
const handlePullRequestSynchronize = async (
  ...
  for (const config of configs) {
    ...
    if (matchedLabel) {
      await octokit.actions.createWorkflowDispatch({
        owner,
        repo,
        workflow_id: config.ifAddWorkflowId,
        ref,
      });
    }
  }
...
const handlePullRequestUnlabeled = async (
  context: Context<"pull_request.unlabeled">,
) => {
  const label = context.payload.label.name;
  const pullRequestNumber = context.payload.pull_request.number;
  context.log.info(
    `Label ${label} was removed from pull request #${pullRequestNumber}`,
  );
  // マージによるクローズであれば、処理対象外
  if (
    context.payload.pull_request.state === "closed" &&
    !!context.payload.pull_request.merged_at
  ) {
    return;
  }
  ...
  await octokit.actions.createWorkflowDispatch({
    owner,
    repo,
    workflow_id: config.ifRemoveWorkflowId,
    ref,
  });
};
...
// biome-ignore lint: ほとんどのContextに共通な処理を行いたいため
...
export const handler = async (event: any) => {
  ...
  if (githubEvent.action === "labeled") {
    await handlePullRequestLabeled(
      context as never as Context<"pull_request.labeled">,
    );
  } else if (githubEvent.action === "unlabeled") {
    await handlePullRequestUnlabeled(
      context as never as Context<"pull_request.unlabeled">,
    );
    ...
    await handlePullRequestSynchronize(
      context as never as Context<"pull_request.synchronize">,
    );
  } else {
